...
interface ListCardProps {
  item: ListItemsType;
  handleComplete: (id: string) => void; // function type
  ...
  return (
    <div className="flex justify-between bg-white border border-gray-200 rounded-lg hover:shadow-md">
      ...
      <div className="flex items-center p-5 gap-5 ">
        ...
        <div
          className={`space-y-2 ${
            item.completed ? "line-through text-gray-500" : "none"
          }`}
        >
          ...
          {dropDown && (
            <ul className="absolute right-0 top-10 w-30 bg-white border border-gray-200 rounded-md shadow-lg z-10">
              <li
                ...
                className="flex items-center gap-2 px-4 py-2 hover:bg-gray-100 transition-colors cursor-pointer"
              >
